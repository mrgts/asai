...
function nListener() {
  window.nRecognition =
  window.nRecognition || window.webkitnRecognition;

  const recognition = new nRecognition();
...
function init() {
  const is_supported = window.nRecognition != null || window.webkitnRecognition != null;
  if (!is_supported) feedback.innerHTML = 'API not supported';

  setColors();
  radioListener();
  setupAnimation();
  is_supported && nListener();
}
